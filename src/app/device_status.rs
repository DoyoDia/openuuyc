...
fn resolve(
    ...
    let activity =
        controller.and_then(|id| crate::controller::gui_connection_activity(id, &device.device_id));
    let mapping =
        controller.and_then(|id| crate::port_mapping::service::status(id, &device.device_id));
    let forwarding =
