...
impl DTLSConn {
    ...
    pub async fn close(&self) -> Result<()> {
        if self
            ...
            .is_ok()
        {
            // A failed close-notify cannot skip local reader/endpoint cleanup.
            // Retain the error, but execute both cleanup steps before returning.
            let notified = self
            ...
            let closed = self.conn.close().await.map_err(Error::from);
            notified.and(closed)?;
        }
    ...
    async fn read_and_buffer(
        ...
        let n = match next_conn.recv(buf).await {
            Ok(n) => n,
            // The truncated datagram was consumed; it is not a transport close.
            Err(util::Error::ErrBufferShort) => return Ok(()),
            Err(err) => {
                // Packet/decryption errors below remain recoverable. A failed
                // underlying receive, unlike a malformed DTLS record, ends the
                // reader and drops decrypted_tx so application readers wake up.
                debug!(
                    "{}: DTLS transport receive ended: {}",
                    srv_cli_str(ctx.is_client),
                    err
                );
