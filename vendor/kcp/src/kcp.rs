...
impl<Output> Debug for Kcp<Output> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Kcp")
            ...
            .field("decrease_fast_ack_after", &self.decrease_fast_ack_after)
...
impl<Output> Kcp<Output> {
    ...
    pub fn set_nodelay(&mut self, nodelay: bool, interval: i32, resend: i32, nc: bool) {
        ...
        match interval {
            interval if interval < 2 => self.interval = 2,
    ...
    /// Limits how many transmissions are eligible for fast resend. Zero keeps
    /// the fast-resend path enabled without an xmit cap.
    #[inline]
    pub fn set_fast_resend_limit(&mut self, limit: u32) {
        self.fastlimit = limit;
    }

    /// Applies UU's KCP v2 high-loss fast-ACK dampening rule.
    #[inline]
    pub fn set_decrease_fast_ack_after(&mut self, enabled: bool) {
        self.decrease_fast_ack_after = enabled;
    }

    /// Smoothed round-trip time maintained by the KCP core.
    #[inline]
    pub fn rx_srtt(&self) -> u32 {
        self.rx_srtt
    }
    ...
    pub fn flush(&mut self) -> KcpResult<()> {
        ...
        for snd_segment in &mut self.snd_buf {
            ...
            if snd_segment.xmit == 0 {
                ...
                && !(self.decrease_fast_ack_after
                    && timediff(self.current, snd_segment.last_fast_resend_ts)
                        < (self.rx_srtt / 2) as i32
                    && snd_segment.fastack < 3)
            {
                if snd_segment.xmit <= self.fastlimit || self.fastlimit <= 0 {
                    ...
                    snd_segment.resendts = self.current + snd_segment.rto;
                    snd_segment.last_fast_resend_ts = self.current;
                    change += 1;
                }
            }
