...
impl AssociationInternal {
    ...
    pub(crate) async fn handle_inbound(&mut self, raw: &Bytes) -> Result<()> {
        // UU multiplexes its `PCK\x02V3` cursor/desktop side channel beside
        // SCTP inside DTLS application data. It is not an SCTP packet and must
        // be dispatched before the SCTP common-header CRC check.
        if raw.starts_with(b"PCK\x02V3") {
            return Ok(());
        }
