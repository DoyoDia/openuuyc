...
pub struct RTCSctpTransport {
    ...
    data_mux: Mutex<Option<Mux>>,
    data_mux_ready: Arc<Notify>,
...
impl RTCSctpTransport {
    pub(crate) fn new(
        ...
        RTCSctpTransport {
            ...
            data_mux: Mutex::new(None),
            data_mux_ready: Arc::new(Notify::new()),
    ...
    pub async fn start(
        ...
        if let Some(net_conn) = &dtls_transport.conn().await {
            let data_mux = Mux::new(MuxConfig {
                conn: Arc::clone(net_conn) as Arc<dyn Conn + Send + Sync>,
                buffer_size: self.setting_engine.get_receive_mtu(),
            });
            let sctp_conn = data_mux
                .new_endpoint(Box::new(|packet| {
                    packet.len() < 4 || packet[..3] != *b"PCK"
                }))
                .await;
            {
    ...
    /// Waits until DTLS application data is available and registers a packet
    /// endpoint beside SCTP on that decrypted transport.
    pub async fn wait_new_data_endpoint(&self, matcher: MatchFunc) -> Option<Arc<Endpoint>> {
        loop {
            let ready = self.data_mux_ready.notified();
            tokio::pin!(ready);
            ready.as_mut().enable();
            {
                let mux = self.data_mux.lock().await;
                if let Some(mux) = mux.as_ref() {
                    return Some(mux.new_endpoint(matcher).await);
                }
            }
            if self.state() == RTCSctpTransportState::Closed {
                return None;
        ...
        loop {
            ...
            let rtc_dc = Arc::new(RTCDataChannel::new(
                DataChannelParameters {
                    ...
                    max_retransmits,
                    priority: Some(dc.config.priority),
                },
