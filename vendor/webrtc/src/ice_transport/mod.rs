...
impl RTCIceTransport {
    ...
    pub async fn get_selected_candidate_pair(&self) -> Option<RTCIceCandidatePair> {
        if let Some(agent) = self.gatherer.get_agent().await {
            if let Some(ice_pair) = agent.get_selected_candidate_pair() {
                let local = RTCIceCandidate::from(&ice_pair.local_candidate());
    ...
    pub(crate) async fn set_gathering_configuration(
        &self,
        policy: crate::peer_connection::policy::ice_transport_policy::RTCIceTransportPolicy,
        ice_servers: &[crate::ice_transport::ice_server::RTCIceServer],
    ) -> Result<()> {
        self.gatherer
            .set_gathering_configuration(policy, ice_servers)
            .await
    }
    ...
    pub async fn role(&self) -> RTCIceRole {
        if self.state() != RTCIceTransportState::New {
            if let Some(agent) = self.gatherer.get_agent().await {
                return if agent.is_controlling() {
                    RTCIceRole::Controlling
                } else {
                    RTCIceRole::Controlled
                };
