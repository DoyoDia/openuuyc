...
impl RTCDataChannel {
    ...
    pub(crate) async fn open(&self, sctp_transport: Arc<RTCSctpTransport>) -> Result<()> {
        if let Some(association) = sctp_transport.association().await {
            ...
            let cfg = data::data_channel::Config {
                ...
                priority: self
                    .priority
                    .unwrap_or(data::message::message_channel_open::CHANNEL_PRIORITY_NORMAL),
    ...
    /// Sends an arbitrary byte sequence with the SCTP text PPID.
    ///
    /// UU's protobuf control channel uses text delivery semantics while its
    /// payload remains binary protobuf and is not necessarily valid UTF-8.
    pub async fn send_text_bytes(&self, data: &Bytes) -> Result<usize> {
        self.ensure_open()?;
