...
pub struct Candidates {
    ...
    pub network_preference: Option<u32>,
    pub vpn_preference: u32,
    pub presume_writable_when_fully_relayed: bool,
...
impl SettingEngine {
    ...
    /// Select the adapter type preferred by UU's ICE controller. `None`
    /// restores the official disabled default.
    pub fn set_network_preference(&mut self, adapter_type: Option<u32>, vpn_preference: u32) {
        self.candidates.network_preference = adapter_type;
        self.candidates.vpn_preference = vpn_preference;
    }

    /// Configure UU's fully-relayed presumed-writable mode. It is disabled by
    /// default and should only be enabled when both sides are known to be
    /// relay-only.
    pub fn set_presume_writable_when_fully_relayed(&mut self, enabled: bool) {
        self.candidates.presume_writable_when_fully_relayed = enabled;
    }

    /// Keep ICE gathering open so later failed-network allocation passes can
    /// publish candidates on the same ICE generation.
    pub fn set_continual_gathering(&mut self, enabled: bool) {
        self.candidates.continual_gathering = enabled;
    }
